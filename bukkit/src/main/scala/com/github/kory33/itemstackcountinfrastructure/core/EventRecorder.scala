...
import cats.effect.kernel.{Clock, GenConcurrent, GenSpawn, GenTemporal, Ref, Resource, Temporal}
...
import shapeless3.deriving.deriving$package.~>
...
object EventRecorder {
  ...
  /**
   * Create a regulated buffer that `massRecords` to `recorder` with sleep of `flushSleep` in between.
   */
  def timeRegulated[F[_]: Temporal](flushSleep: FiniteDuration)
                                   (recorder: EventRecorder[F]): Resource[F, EventRecorder[F]] = {
    Resource.eval[F, Ref[F, FiniteDuration]](Clock[F].realTime.flatMap(Ref[F].of)).flatMap { lastFlushedRef =>
      ...
      for {
        queue <- Resource.eval[F, CEQueue[F, ItemStackMovementEvent]](CEQueue.unbounded)
        ...
        new EventRecorder[F] {
          override def record(event: ItemStackMovementEvent): F[Unit] = queue.offer(event)
          override def massRecord(events: List[ItemStackMovementEvent]): F[Unit] = events.traverse(queue.offer).void
        }
      }
    }
  }
  ...
  /**
   * Create a resource for a synchronized recorder that allows writing in [[SyncIO]] context.
   */
  def synchronize[F[_]](asyncRecorder: EventRecorder[F])(trans: SyncIO ~> F)
    ...
    Resource.make(makeQueueRef) { queue =>
      for {
      ...
      new EventRecorder[SyncIO] {
        ...
        override def massRecord(events: List[ItemStackMovementEvent]): SyncIO[Unit] =
